...
CPYTHON_DISTRIB_URL = f"https://www.python.org/ftp/python/{CPYTHON_VERSION}/{CPYTHON_DISTRIB_NAME}.zip"
...
def main():
    ...
    print("### Patch parsec.__version__ in repo ###")
    ...
    if not WHEELS_DIR.is_dir():
        ...
        run(f"{ TOOLS_VENV_DIR / 'Scripts/pip' } wheel {PARSEC_REPO_DIR}[core] --wheel-dir {WHEELS_DIR}")

    # Now we actually generate the build target

    target_dir = BUILD_DIR / f"parsec-{parsec_verbose_version}-{get_archslug()}"
    ...
    print(f"### Move site-packges to embedded distribution ###")
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "FileVersion",      "{parsec_verbose_version}\\0"
            ...
            VALUE "ProductVersion",   "{parsec_verbose_version}\\0"
        END
    END
    BLOCK "VarFileInfo"
    BEGIN
